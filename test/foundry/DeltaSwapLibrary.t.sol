...
contract DeltaSwapLibraryTest is Test {
    ...
    function testCalcTradeLiquidity(uint128 amount, bool isToken0) public {
        ...
        uint256 tradeLiquidity = Math.calcTradeLiquidity(amount0, amount1, reserve0, reserve1);
        assertEq(tradeLiquidity, Math.sqrt(uint256(amount)*amount));
        assertEq(Math.calcTradeLiquidity(0, 0, reserve0, reserve1), 0);
    }

    function testCalcEMAFail() public {
        vm.expectRevert("EMA_WEIGHT > 100");
        Math.calcEMA(1, 1, 101);
    }
    ...
    function testCalcEMA0pctWeight() public {
        ...
        uint256 ema = Math.calcEMA(last, emaLast, emaWeight);
        assertEq(ema, 0);

        last = 1 * 1e18;
        ema = Math.calcEMA(last, emaLast, emaWeight);
        assertEq(ema, last);
